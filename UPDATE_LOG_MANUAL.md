...
### 使用反馈
- （当时未回填；2026-07-06 那批条目的使用反馈始终空着，此后几次修改也没再写这一节，直到 07-24 第 30/31 次才恢复记录）
...
### 使用反馈
- **2026-07-24 用户在 iPhone 11 主屏 App 上确认更新成功**（用户原话「搞定了」）。这次没有删除主屏图标、没有重新添加、没有重填 GitHub token——即本次要解决的核心痛点已经在真机上验证消除。
- 注意：本条是用户回报的结果，不是我在他设备上逐步观察到的；横幅出现、点击、刷新等中间步骤只在 Chrome 和线上 Pages 环境实测过（见第 31 次条目），iOS Safari 主屏容器里的具体表现以用户回报为准。
- 从这一版起恢复填写「使用反馈」一节（07-06 那批条目的这一节始终空着）。
...
## 2026-07-24 第 31 次修改（版本 2026-07-24.1）

### 本次目标
- 在界面上显示版本号，让「我现在用的是哪一版」随时可查、更新有没有生效可验证
- 把版本规则在文档里写清楚
...
**① 版本号规则**
- `APP_VERSION` 从 `"2026-07-24"` 改为 `"2026-07-24.1"`，格式 **`日期.当日第几版`**——只写日期的话同一天发两次根本分不出来
- 新增 `APP_RELEASE_NOTE`：一句话说明本版改了什么，显示在「关于」页

**② 侧栏常驻版本徽章**
- 标题下方一个小胶囊 `v2026-07-24.1`，点一下直接跳「设置 → 关于」
- 有新版本待安装时变绿并显示「· 有新版本」（`.brand-version.has-update`）
- 桌面和手机都可见（侧栏在移动端是 sticky 顶栏，不是隐藏的）

**③ 新增「设置 → 关于」分区**
- 原本版本信息塞在「教材与进度」最底下，位置不合理也不好找；现在独立成 `ℹ 关于` 分区，教材页只留一句指路
- 十项信息：应用版本 / 本版内容 / 缓存版本 / 运行方式（主屏 App vs 浏览器标签页）/ 上次检查更新 / 当前课次 / 题库规模 / 每轮题量 / 答题记录条数 / 同步云端 Gist
- 「运行方式」用 `navigator.standalone` + `matchMedia("(display-mode: standalone)")` 判断，能直接看出这个窗口是不是主屏 App
- 页内写明**怎么确认更新成功**：更新后回来看「应用版本」是否变成新的日期序号

**④ 缓存版本向 SW 实时查询（不抄常量）**
- SW 新增 `GET_VERSION` 消息处理，用 `event.ports[0]` 回 `CACHE_NAME`
- 页面 `requestSwCacheName()` 用 `MessageChannel` 单次问答获取
- 理由：在 app.js 里另抄一份 `CACHE_NAME` 常量迟早和 SW 里的漂移，问 SW 拿到的才是真正生效的那个
- 同时记录 `lastUpdateCheckAt`，在「关于」里显示上次检查时间

**⑤ 【顺带发现并修复】阅读卡 id 跨来源撞车（老 bug）**
- 做「题库规模」统计时发现篇数显示 49，而实际是 21 篇内置 + 30 篇 AI = 51
- 查下来是 `buildLessonReadingCard()` 只给 `textbook` 加了 `tb-` 前缀，**`ai-generated` 和内置课文共用同一命名空间**，`lesson` + `index` 相同就撞车：`jp-reading-minna-5-4`、`jp-reading-minna-10-9` 等 **8 张卡 id 完全重复**
- 后果不只是数字不准：两张内容不同的句子卡**共用一份 SRS 进度**（答了内置那张，AI 那张也被标记成已复习），而且 `getCard(id)` 只能取到第一张，另一张永远出不来
- 这是 07-14 第 29 次加入 AI 短文时引入的，与本次拆句无关；拆句只是让篇数统计把它暴露出来
- 修复：抽出 `readingIdPrefix(source)`，`ai-generated` 用 `ai-` 前缀。修复后 224 张卡 id 全部唯一，篇数正确显示 51
- 副作用：AI 短文卡的 id 变了，这些卡上原有的复习进度会作废——但那份进度本来就是和内置卡混在一起的脏数据，作废反而是对的

### 未完成/暂缓
- 语法目录仍无产出题型（承第 30 次）
- 数据保存安全（deviceId/revision/本地恢复点）

### 测试结果
- `node test.js`：**55 passed, 0 failed**（54 → 55）
  - 新增「阅读卡 id 跨来源全局唯一」回归测试：断言 `japaneseReadingLabCards + aiReadingCards` 无重复 id、两来源篇数相加等于合并后篇数、AI 卡带 `ai-` 前缀
- 浏览器实测（本地 8787，清空 SW 和缓存后全新加载）：
  - 侧栏徽章显示 `v2026-07-24.1`，点击跳转到「关于」分区
  - 从 SW 问到的缓存版本 `triad-learning-trainer-v13`，与 service-worker.js 里一致
  - 「关于」十项信息齐全，题库规模显示「661 词 · 87 条语法 · 51 篇短文（拆成 224 张单句卡）」
  - 375×812 移动端视口：徽章可见可点，关于页栅格降为单列，无横向溢出
  - 控制台无错误
- **线上 Pages 真实更新验证**（非本地模拟）：浏览器上留着上一版 v12 的 SW，打开线上站后自动检测到新版本 → 徽章变成「v2026-07-24.1 · 有新版本」→ 点「立即更新」→ 页面刷新 → 徽章恢复正常、横幅消失、缓存版本从「（未启用 Service Worker）」变为 `triad-learning-trainer-v13`
  - 中间那个「未启用 Service Worker」不是 bug：旧的 v12 worker 没有 `GET_VERSION` 处理器，问了不回，更新到 v13 后才有回应

### 使用反馈
- **2026-07-24 用户确认手机端已更新成功**（用户原话「搞定了」），本次版本可见性改动同批上线。
- 待观察：用户尚未反馈是否用侧栏徽章/「关于」页核对过版本号，以及「运行方式」在 iOS 主屏 App 里是否正确显示为「主屏 App（独立窗口）」——这一项依赖 `navigator.standalone`，只在真机主屏容器里才能验证，桌面 Chrome 测不出来。

### 当前状态
- 版本 **2026-07-24.1**，SW 缓存 **v13**
- 版本号可在侧栏徽章和「设置 → 关于」两处查看
- 阅读卡 224 张，id 全局唯一

### 下一步建议
- ~~手机更新后先看侧栏徽章是不是 `v2026-07-24.1`~~ → 用户已确认更新成功（见使用反馈）
- 以后每次发版记得同时改 `APP_VERSION`、`APP_RELEASE_NOTE`、`CACHE_NAME` 三处
- 手机上把 Gist ID 填成 `c80f075251422031816a8c400057ba93`（合并后的权威云端），确认课次进度和 143 条历史都同步过来
- 内容侧的下一件事：给语法目录加产出题型（承第 30 次未完成项），补上产出题占比到不了 60% 的供给缺口

---

## 2026-07-24 第 32 次修改（版本 2026-07-24.2）

### 本次目标
- 补上语法目录的产出题型（承第 30/31 次未完成项）
- 确认手机同步 Gist 配置是否需要改
...
**① 语法变形题（drills）**
- 语法目录原本 3 种题型（选意思 / 选接续 / 看中文选例句）**全是选择题**，只能练"认得出"，练不了"写得出"——而实测产出题正确率比选择题低约 40 个百分点，`conjugation` 更是最弱的标签（43%）
- 语法条目新增可选字段：`drills: [[题面, 答案, 说明]]` + `drillLabel`（题面里"改成什么形"）+ `drillHint`（输入框提示）
- `buildGrammarCardsFromBank()` 新增第 4 种题型：`type: "input"` 的变形题，题面形如「把「行きます」改成 ない形。」，带 `production` / `recall` / `conjugation` 标签
- **共 51 道，覆盖 10 个句型**：
  - 動詞て形 13 道（Ⅰ类五种音变 + 行きます 特例 + Ⅱ/Ⅲ类）
  - 動詞ない形 15 道（含 買います→買わない 的 い→わ 特例、来ます→来ない）
  - Vたいです / Vてください / Vてもいいです / Vてはいけません / Vてから / Vないでください / Vなければなりません / Vなくてもいいです 各 2-3 道
- 答案都是短词（`書かない`、`帰ってから`），手机上打得动，自动判分也准；解析里带变形规则说明（如「Ⅰ类特例：い→わ，不是「買あない」」）
- 用的动词全部核对过在第 1-17 课词表范围内

**② 「产出题占比」改名为「产出题优先度」**
- 原来的下拉写「30% / 60% / 80%」，暗示是硬配额，但实测队列里只能到 38-46%——因为供给不够（词汇每词 3 种题型只有 1 种要写）
- 改成「低 / 中 / 高」，并在设置页如实说明：这是排序优先度不是硬配额，实际占比约 40-50%，调到"高"也超不过供给上限
- 同时显示当前课次范围内的语法变形题条数（51 道），是个可核对的具体数字

**③ 供给统计口径修正（过程中的自我纠错）**
- 第一版写了 `grammarProductionShare()` 遍历整个 `grammarCatalog` 算百分比，得出 16%
- 但用户实际拿到的不是这个数：变形题集中在 13-17 课，经课次门控和 `prioritizeByProgress` 截断后，进池的比例是 37%、最终队列 38%
- 同一件事在三个环节有三个数（16% / 37% / 38%），报哪个都容易误导
- 改成 `grammarDrillCount()` 报**绝对条数**（当前课次范围内 51 道）——稳定、可核对、不会因为统计口径产生歧义

### 手机同步 Gist：确认无需修改
- 上次建议把手机 Gist ID 改成 `c80f0752…`，用户反馈没改、还是旧的
- 查证结论：**不用改，本来就是对的**。那个 Gist 就是手机自己建的（07-24 12:40 点「上传本机」时 `createGitHubGist()` 新建的），后来合并三份数据时按用户选择也是写进这一个
- 证据：`c80f0752` 在 07-24 13:29 和 13:44 有新修订（手机更新后自动同步产生），内容为 143 条历史 / 76 张卡进度 / 生词 19 / 语法 12，且 `lessonProgress.currentLesson = 17`、`lessonBaseline = 17`（说明新版 app 的课次基线迁移已在手机上跑过并推回云端），存档里自报的 `gitSync.gistId` 也正是 `c80f0752`
- 另外两个旧 Gist（`bfff6c1f` 停在 07-13、`3a4f05e1` 停在 07-06）没有新写入，确认已停用

### 未完成/暂缓
- 词汇产出题供给仍是每词 1/3（可考虑加"看中文写假名读音"提高到 1/2）
- 数据保存安全（deviceId/revision/本地恢复点）

### 测试结果
- `node test.js`：**57 passed, 0 failed**（55 → 57）
  - 新增「语法目录：变形题数据结构完整」：断言 ≥8 条语法带 drills、每条有 drillLabel、题面≠答案、总数 ≥40、ない形覆盖 い→わ 特例和 Ⅲ类
  - 新增「语法出题器：产出题真的被生成出来」：断言 input 题 ≥20、id 唯一、题面含"改成"、accepted 含正确答案、带 production/conjugation 标签
- 浏览器实测：
  - 真做一道变形题：题面「把「帰ります」改成「做完…之后」的说法。」、提示「用 Vてから 说出来」，输入 `帰ってから` 判对、输入错误答案判错，解析显示「り→って 后接」
  - 8 轮队列累计：词汇产出占比 46%、语法产出占比 38%（改动前语法产出题为 0）
  - 设置页供给说明显示 51 道，与手动核对一致
  - 控制台无错误

### 当前状态
- 版本 **2026-07-24.2**，SW 缓存 **v14**
- 语法题库：312 道（原 261 道选择题 + 51 道变形题）
- 每轮 20 题里产出题约 5 道，其中语法变形题约 2 道
- 同步：单一 Gist `c80f0752…`，手机和电脑都指向它，无需改配置

### 下一步建议
- 手机更新后到「设置 → 关于」确认版本是 `2026-07-24.2`
- 重点练 ない形那 15 道变形题——`jp-grammar-007`「把「行きます」变成 ない形」是全库 0 对 2 错的三张卡之一，现在有系统性的配套练习了
- 词汇侧也可以加一种产出题型，把产出占比从 40-50% 再往上推

---

## 2026-07-24 第 33 次修改（版本 2026-07-24.3）

### 本次目标
- 补上词汇侧的产出题型，把产出练习的供给缺口填平（承第 32 次未完成项）
...
**① 新题型：看汉字写读音（`-r`）**
- 题面「「覚えます」怎么读？（写假名）」→ 答案 `おぼえます`，`type: "input"`
- 只对**汉字词**出（`reading !== word`）：假名词的写法就是读音，出了没意义。661 词里 474 个是汉字词（72%）
- 标签 `kanji-reading` + `production` + `recall`
- 和已有的「看中文写日语」（`-w`）练的是两种不同能力：`-w` 是"这个意思用日语怎么说"，`-r` 是"这个汉字念什么"。原本 `kanji-reading` 只有选择题（看假名选写法）能练，写不出来照样能选对
- 顺带给 `-w` 补上 `production` 标签，两种产出题标签口径统一

**② 供给和实测占比**
- 词汇题库 1703 → **2270 道**（新增 474 道读音题 + 原有）；产出/再认从 1135/1135 = **正好各占一半**
- 10 轮队列实测：**词汇产出占比 46% → 51%**，语法 38% → 41%
- 设置页供给说明同步更新为实测值，不再写笼统的「40-50%」

### 未完成/暂缓
- 数据保存安全（deviceId/revision/本地恢复点）——现在是唯一剩下的老项目了

### 测试结果
- `node test.js`：**58 passed, 0 failed`**（57 → 58）
  - 新增「词汇出题器：产出题占到一半」：断言产出占比 ≥45%、`-w` ≥500 道、`-r` ≥400 道、读音题题面含"怎么读"、带 kanji-reading/production 标签、**假名词不出读音题**、所有产出题的 accepted 都含正确答案
- 浏览器实测：
  - 抽到「「乗り換えます」怎么读？（写假名）」，输入 `のりかえます` 判对、输入错误答案判错
  - 设置页显示「词汇题里产出题约占一半（看中文写词 + 看汉字写读音），语法变形题共 51 道。实测词汇约 51%、语法约 41%」
  - 「关于」页版本 `2026-07-24.3`、本版内容已更新
  - 控制台无错误

### 当前状态
- 版本 **2026-07-24.3**，SW 缓存 **v15**
- 词汇题 2270 道（产出 1135 / 再认 1135），语法题 312 道（变形 51 / 选择 261）
- 每轮 20 题里产出题约 6 道（词汇 ~3 + 语法 ~2 + 组句 ~1）
- 产出练习的供给缺口已填平：词汇 51%、语法 41%，均接近各自的供给上限

### 下一步建议
- 手机更新后确认版本 `2026-07-24.3`
- 练一段时间后重跑弱项分析，看 `input` 正确率（合并数据时是 46%）和 `kanji-reading`（73%）有没有起色——这是判断这三次改动是否真的有效的唯一依据
- 剩下的老项目只有数据保存安全（deviceId/revision/本地恢复点）了
